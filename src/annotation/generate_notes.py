...
def generate_dataset(n:int) -> list:
    prompts = []
    for i in range(1,n):
        prompt = build_prompt_with_sampling(seed=np.random.randint(i+100))
...
if __name__ == "__main__":
    ...
    example_prompt = build_prompt_with_sampling(seed=np.random.randint(1,100))
    ...
    if args.test:
        output_suffix = "test_run_yet_another_prompt"
    else:
